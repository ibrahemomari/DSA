...
function transformToLis(obj){
  // Solution code here...
  let newArray=Object.entries(obj).map(el=>{
    return  `<li>${el.join(': ')}</li>`
})
return newArray;
}
...
const totalSum = (input) => {
  // Solution code here...
  let sum= input.reduce((x,y) => x + y.reduce((x,y)=> x + y ,0),0); // error
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArray=input.map(el=>el.filter(el=>!(el%5)));
  let checkStr=newArray.map(el=>el.filter(el=>typeof(el) !=='string'));
  let result=checkStr.map(el=>el.map(el=>Math.pow(2,el)));
  return result;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newArray= data.filter(el=>el.gender==='female' || el.gender==='male').map(el=>el.name).join(' and ');
  return newArray;
};
