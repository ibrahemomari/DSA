...
const forLoopTwoToThe = (arr) => {
  ...
  for(let num in arr){
    let power=arr[num];
    newArray.push(Math.pow(2,power));
  }
  return newArray;
};
...
const evenOdd = (arr) => {
  // Solution code here...
  let newArray=arr.map(el=>{
    if(el%2==0 && isNaN(el)==false){
      return 'even';
    }else if(el%2!=0 && isNaN(el)==false){
      return 'odd';
    }else if(isNaN(el)==true){
      return 'N/A';
    }
  })
 return newArray;
...
const extractStats = (arr) => {
  // Solution code here...
   // Solution code here...
   let newArray=arr.map(el=>{
    return {name:el.stat.name,total:el.effort+el.baseStat};
  })
  return newArray;
};
